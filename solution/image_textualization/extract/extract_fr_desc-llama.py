...
def main(args):
    ...
    with open(args.input_file, 'r') as input_file:
        with open(args.output_file, 'a') as output_file:
            llm = LLM(model="./llama3/Meta-Llama-3-70B-Instruct", tensor_parallel_size=4,enforce_eager=True,gpu_memory_utilization=0.9) # todo 与gpu数保持一致
            ...
            for index in range(0, len(prompts_list), multi_num):
                ...
                img_name_list = image_name_list[index: index + multi_num]
                des_list = description_list[index:index + multi_num]


                for response,img_name,des in zip(response_list,img_name_list,des_list):
                    prompt = response.prompt
                    # obj_extr = response[0].outputs[0].text
                    obj_extr = response.outputs[0].text
