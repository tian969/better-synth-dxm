...
additional_info = """
###TASK Description###
You are a helpful language assitant. Imagine visualizing an image based on its description. Now, your task is to make the Original Description more detailed and accurate. You'll need to use the subsequent provided Objects, along with the corresponding extra information of Relative Spatial Positioning, Relative Distance from the Lens, and Relative Size Proportion in Images (Percentage), to better assist you in adding these Objects to the original description. Additionally, Hallucinations are also provided below, you need delete Hallucinations from the original description too.
...
4. Your response should only contain the modified description, do not contain other things (eg. your reason for modifying).
...
-----
# Example 2:
%%%The Original Description:%%% At the center of the frame is a **black Rolex clock** mounted on a **black pole**. The clock has a **white face** with **black hands**, indicating the time.\n\nBehind the clock, there are a **brown tree trunk** with a rough texture and a traffic light. Further back, there's a **white building** with a **red tile roof**, possibly a hotel as indicated by the sign that reads "**HOTEL**".\n\nIn front of the building, there's a **white car** parked, perhaps belonging to one of the hotel guests. Beyond this immediate scene, there's a street with a **white crosswalk**, where a bus is parked.

Hallucinations: a traffic light; a bus
@@@
...
2. In The Original Description, "there's a street with a **white crosswalk**, where a bus is parked" should be modified to "there's a street with a **white crosswalk**", since "a bus" is a hallucincation that needs to be deleted.
@@@
...
%%%Your Modified Description:%%% At the center of the frame is a **black Rolex clock** mounted on a **black pole**. The clock has a **white face** with **black hands**, indicating the time.\n\nBehind the clock, there's a **brown tree trunk** with a rough texture. Further back, there's a **white building** with a **red tile roof**, possibly a hotel as indicated by the sign that reads "**HOTEL**".\n\nIn front of the building, there's a white car parked, perhaps belonging to one of the hotel guests. Closer to the camera, there is a parked silver car near a stop sign. Beyond this immediate scene, there's a street with a white crosswalk. At a distance from the camera, you can see a person walking on the sidewalk, occupying a very small part of the entire image.
...
------
# Example4:
%%%The Original Description:%%% The main focus is a bustling street, lined with a variety of buildings and shops, including a post office. A yellow taxi, the only vehicle visible in the frame, is driving down the road.

Hallucinations: a post office
@@@
1. In The Original Description, "The main focus is a bustling street, lined with a variety of buildings and shops, including a post office." should be modified to "The main focus is a bustling street, lined with a variety of buildings and shops".
@@@
...
def main(args):
    ...
    with open(args.input_file, 'r') as input_file, open(args.output_file, 'w') as output_file:
        ...
        llm = LLM(model="/home/disk1/LLMs/Meta-Llama-3___1-8B-Instruct", tensor_parallel_size=4,enforce_eager=True,gpu_memory_utilization=0.9)
        prompts_list = []
        images_list = []
        desc_list = []
        for i, line in enumerate(tqdm(input_file, total=total_lines)):
            ...
            if add_obj_num < 2:
                ...
                if max_depth == min_depth:
                    ...
                    hallucinations_str = '; '.join(hallucinations)
                    hall_str = f"Hallucinations: {hallucinations_str}\n"
                    output_str = output_str + hall_str
        ...
        # 将不需要调用llm的内容写入文件中
        for data in tqdm(desc_list, desc="writing data directly"):
            json.dump(data, output_file)
            output_file.write('\n')

        sampling_params = SamplingParams(temperature=0.75, top_p=0.95, max_tokens=2048, stop=stop_tokens)
        multi_num = 100  # todo 并发数
        for index in range(0, len(prompts_list), multi_num):
            prompt_list = prompts_list[index: index + multi_num]
            response_list = llm.generate(prompt_list, sampling_params)
            img_name_list = images_list[index: index + multi_num]

            for image, desc in tqdm(zip(img_name_list, response_list), total=len(img_name_list),
                                    desc="Generate refined desc"):
                description = desc.outputs[0].text
                data = {
                    "image": image,
